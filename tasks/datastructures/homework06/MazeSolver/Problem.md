...
* `Maze()` - A class that will contain a two-dimensional grid and will be a representation of the maze.

* `static Maze? FromString(string mazeStr)` - Create a maze object from a string. If the string is empty or contains invalid characters, it will return null.

* `void SetGrid(List<List<string>> grid)` - Sets the grid and will recompute the start, end, width, and height.

* `List<List<string>> GetGrid()` - Will return the grid of characters(strings).

* `string DrawGrid()` - Draw the grid as a string, same the string format.

* `Point GetStart()` - Gets a x,y tuple of the starting position.

* `void SetStart(Point p)` - Sets the starting position of the maze.

* `Point GetExit()` - Gets a x,y tuple of the end position.

* `void SetExit()` - Sets the end position of the maze.

* `int GetHeight()` - Gets the height of the maze.

* `int GetWidth()` - Gets the width of the maze.
